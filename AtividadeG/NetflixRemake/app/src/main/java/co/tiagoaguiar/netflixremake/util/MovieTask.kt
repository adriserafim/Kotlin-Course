...
class MovieTask(private val callback: Callback) {
            ...
            try {
               val requestURL = URL(url)
                ...
                if (statusCode == 400) {
                    ...
                    val jsonAsString = toString(buffer) // Aqui eu já tenho o JSON
                ...
                val movieDatail = toMovieDetail(jsonAsString)
                handler.post {
                    callback.onResult(movieDatail)
                }
            } catch (e: IOException) {
    ...
    private fun toMovieDetail(jsonAsString:String) : MovieDetail {
        ...
        for(i in 0 until jsonMovies.length()) {
            ...
            val similaCoverUrl = jsonMovie.getString("cover_url")
            val m = Movie(similarId, similaCoverUrl)
            similars.add(m)
        }
